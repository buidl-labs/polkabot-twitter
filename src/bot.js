...
async function postTopThreeBackedValidatorsByNominatorsOnTwitter(){
  ...
  fs.readdir(__dirname + '/images', (err, files) => {
    if (err) {
      ...
      const fileImages = files.filter(file => validator[0].stashId === file.split(".")[0]);
  ...
  function postOnTwitter(images, validator) {
    ...
    Promise.all([
      ...
      bot.post('statuses/update', {
        status: `Top 3 validators on the Polkadot Network for the previous era, ranked Nominators:
      🥇${validator[0].name}-Backers:${validator[0].noOfNominators}
      🥈${validator[1].name}-Backers:${validator[1].noOfNominators}
      🥉${validator[2].name}-Backers:${validator[2].noOfNominators}
  ...
  fs.readdir(__dirname + '/images', (err, files) => {
    if (err) {
      ...
      const fileImages = validator.map(validator => {
        const id = files.find(imageName => imageName.split('.')[0] === validator.stashId)
        return id
      })
      console.log("fileImages", fileImages)
...
function uploadImages(images, validator) {
  ...
  Promise.all([
    ...
    console.log('list', list);
    bot.post('statuses/update', {
      status: `Top 3 validators on the Polkadot Network for the previous era, ranked by pool reward:
      🥇${validator[0].name} - ${validator[0].poolRewardWithCommission.toFixed(3)} KSM
      🥈${validator[1].name} - ${validator[1].poolRewardWithCommission.toFixed(3)} KSM
      🥉${validator[2].name} - ${validator[2].poolRewardWithCommission.toFixed(3)} KSM
      See how much you could be earning on https://polkanalytics.com/`,
      media_ids: list
    }).catch(err => {
      console.log("twit bot err", err);
